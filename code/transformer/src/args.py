...
def build_parser():
	...
	parser.add_argument('-eval_last_n', type=int, default= 1, help='Last n epochs to be evaluated') # <dev/test/gen>_always parameter will override this
	...
	parser.add_argument('-d_model', type=int, default=256, help='the number of expected features in the encoder inputs') #768? features of BERT? HAS TO BE 300 if using word2Vec
	...
	parser.add_argument('-freeze_transformer_encoder', dest='freeze_transformer_encoder', action='store_true', help='Freeze Transformer Encoder weights')
	parser.add_argument('-no-freeze_transformer_encoder', dest='freeze_transformer_encoder', action='store_false', help='Train Transformer Encoder weights')
	parser.set_defaults(freeze_transformer_encoder=False)
	parser.add_argument('-freeze_transformer_decoder', dest='freeze_transformer_decoder', action='store_true', help='Freeze Transformer Decoder weights')
	parser.add_argument('-no-freeze_transformer_decoder', dest='freeze_transformer_decoder', action='store_false', help='Train Transformer Decoder weights')
	parser.set_defaults(freeze_transformer_decoder=False)
	...
	parser.add_argument('-epochs', type=int, default=150, help='Maximum # of training epochs')
